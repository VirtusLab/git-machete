...
long_docs: Dict[str, str] = {
	"add": """
		...
		<b>Usage:</b> git machete add[-o|--onto=<target-upstream-branch>][-R|--as-root][-y|--yes][<branch>]
	...
	"advance": """		.green {color:green}

		<b>Usage:</b> git machete advance[-y|--yes]
		...
		As an example, if `git machete status` --color=never--list-commitsis as follows:
		<dim>
		  master
		  |
		  m-develop *
		...
		<b>Options:</b>	
			-y,--yes	Don't ask for confirmation whether to fast-forward the current branch or whether to slide-out the downstream. Fails if the current branch has more than one [32mgreen-edge[0m downstream branch.	""",
	"anno": """

		<b>Usage:</b> git machete anno[-b|--branch=<branch>][<annotation text>]git machete anno -H|--sync-github-prs
		...
		a GitHub API token with `repo` scope is required, seehttps://github.com/settings/tokens. This will be resolved from the first of:
		...
		To do this, run `git config` --local--editand add the following section:[machete "github"]organization=<organization_name>repository=<repo_name>remote=<remote_name>
	...
	"clean": """
		...
		<b>Usage:</b> git machete clean[-c|--checkout-my-github-prs][-y|--yes]
	...
	"delete-unmanaged": """
		...
		<b>Usage:</b> git machete delete-unmanaged[-y|--yes]

		Goes one-by-one through all the local git branches that don't exist in the definition file,
		and ask to delete each of them (with `git branch` -dor `git branch` -D) if confirmed by user.
		No branch will be deleted unless explicitly confirmed by the user (or unless `-y/--yes` option is passed).
		...
		See fork-point for more details.

		<b>Options:</b>	
			-y,--yes	Don't ask for confirmation.	""",
	"diff": """

		<b>Usage:</b> git machete d[iff][-s|--stat][<branch>]
		...
		See fork-point for more details on meaning of thefork point.
	...
	"discover": """
		...
		<b>Usage:</b> git machete discover[-C|--checked-out-since=<date>][-l|--list-commits][-r|--roots=<branch1>,<branch2>,...][-y|--yes]
	...
	"fork-point": """		.green {color:green}.yellow {color:#FFBF00}

		<b>Usage:</b> git machete fork-point[--inferred][<branch>]git machete fork-point --override-to=<revision>|--override-to-inferred|--override-to-parent[<branch>]git machete fork-point --unset-override[<branch>]
		...
		Also, `reapply` , `slide-out` , `squash` , and `update` allow to specify the fork point explicitly by a command-line option.
		 ...
		 `git machete` fork-pointis different (and more powerful) than `git` merge-base--fork-point,
		...
		With `--inferred` , displays the commit that `git machete` fork-pointinfers to be the fork point of <branch>.
		If there is NO fork point override for <branch>, this is identical to the output of `git machete` fork-point.
		If there is a fork point override for <branch>, this is identical to the what the output of `git machete` fork-pointwould be if the override was NOT present.

		With `--override-to-inferred` option, overrides fork point of the <branch> to the commit that `git machete` fork-pointinfers to be the fork point of <branch>.
		Note: this piece of information is also displayed by `git machete status` --list-commitsin case a [33myellow[0m edge occurs.
	...
	"format": """
		...
		Branches `adjust-reads-prec` , `edit-margin-not-allowed` and `grep-errors-script` are direct downstream branches for `develop` . `block-cancel-order` is a downstream branch of `adjust-reads-prec` , `change-table` is a downstream branch of `block-cancel-order` and so on.
	...
	"github": """
		...
		<b>Usage:</b> git machete github <subcommand>
		...
		To do this, run `git config` --local--editand add the following section:[machete "github"]organization=<organization_name>repository=<repo_name>remote=<remote_name>
		 ...
		 `anno-prs` :Annotates the branches based on their corresponding GitHub PR numbers and authors.
		Any existing annotations are overwritten for the branches that have an opened PR; annotations for the other branches remain untouched.
	...
	"go": """
		...
		<b>Usage:</b> git machete g[o]<direction>
	...
	"help": """
		...
		<b>Usage:</b>gitmachetehelp[<command>]
	...
	"hooks": """
		...
		If machete-pre-rebase returns zero, the execution flow continues to `git rebase` , which may also run `pre-rebase` hookif present. `machete-pre-rebase` is thus always launched before `pre-rebase` .
			* 
		...
		Please seehook_samplesfor examples.
		An example of using the standard git `post-commit` hookto `git machete add` branches automatically is also included.	""",
	"is-managed": """

		<b>Usage:</b> git machete is-managed[<branch>]
	...
	"list": """
		...
		<b>Usage:</b> git machete list <category>
	...
	"log": """
		...
		<b>Usage:</b> git machete l[og][<branch>]
	...
	"reapply": """
		...
		<b>Usage:</b> git machete reapply[-f|--fork-point=<fork-point-commit>]
	...
	"show": """
		...
		<b>Usage:</b> git machete show <direction>[<branch>]
	...
	"slide-out": """
		...
		<b>Usage:</b> git machete slide-out[-d|--down-fork-point=<down-fork-point-commit>][--delete][-M|--merge][-n|--no-edit-merge|--no-interactive-rebase][<branch>[<branch>[<branch> ...]]]
		...
		After running `git machete` slide-outadjust-reads-precblock-cancel-orderthe tree will be reduced to:
		<dim>
		  develop
		      change-table
		          drop-location-type
		...
		Example: `GIT_MACHETE_REBASE_OPTS="--keep-empty` --rebase-merges"git macheteslide-out.	""",
	"squash": """

		<b>Usage:</b> git machete squash[-f|--fork-point=<fork-point-commit>]
	...
	"status": """		.green {color:green}.grey {color:dimgrey}.red {color:red}.yellow {color:#FFBF00}

		<b>Usage:</b> git machete s[tatus][--color=WHEN][-l|--list-commits][-L|--list-commits-with-hashes][--no-detect-squash-merges]
		...
		and the fork point of the downstream branch is equal to the upstream branch tip,
			*  [2mgrey/dimmed edge[0m means that the downstream branch has been merged to the upstream branch,
		...
		With `--color=always` , git machete always emits colors and with `--color=auto` , it emits colors only when standard output is connected to a terminal. `--color=auto` is the default. When colors are disabled, relation between branches is represented in the following way (not including the hash-comments):
		<dim>
		  <branch0>
		  |
		...
		Only considerstrict(fast-forward or 2-parent) merges, rather than rebase/squash merges, when detecting if a branch is merged into its upstream (parent).
		...
		With `machete.status.extraSpaceBeforeBranchName` config set to `true` :develop
		│
		├─ feature_branch1
		│
		└─ feature_branch2	""",
	"traverse": """		.green {color:green}.grey {color:dimgrey}.red {color:red}.yellow {color:#FFBF00}
		...
		--- equivalent to `git machete update` with no `--fork-point` option passed;
			...
			* asks the user whether to `git reset` --keepthe branch to its remote counterpart
		...
		<b>Options:</b>	
			...
			-n	If updating by rebase, equivalent to `--no-interactive-rebase` . If updating by merge, equivalent to `--no-edit-merge` .	
			--no-detect-squash-merges		Only considerstrict(fast-forward or 2-parent) merges, rather than rebase/squash merges, when detecting if a branch is merged into its upstream (parent).	
	...
	"update": """
		...
		<b>Usage:</b> git machete update[-f|--fork-point=][-M|--merge][-n|--no-edit-merge|--no-interactive-rebase]
