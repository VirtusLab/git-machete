...
def launch(orig_args: List[str]) -> None:
    ...
    try:
        ...
        if cmd == "add":
            ...
            def warn_on_deprecation(*, flag: str, revision: str, revision_str: str) -> None:
                if upstream:
                    print()
                    warn(
                        f"`git machete fork-point {flag}` may lead to a confusing user experience and is deprecated.\n\n"
                        f"If the commits between <b>{upstream}</b> (parent of <b>{branch}</b>) and {revision_str} "
            ...
            if cli_opts.opt_inferred:
                ...
                warn_on_deprecation(
                    flag="--override-to=...",
                    revision=cli_opts.opt_override_to,
                    revision_str=f"<b>{cli_opts.opt_override_to}</b>")
                ...
                warn_on_deprecation(
                    ...
                    revision=fork_point,
                    revision_str=f"inferred commit (<b>{fork_point}</b>)")
