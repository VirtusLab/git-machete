...
def create_cli_parser() -> argparse.ArgumentParser:
    ...
    anno_parser.add_argument('--sync-gitlab-mrs', action='store_true', default=argparse.SUPPRESS)
    ...
    try:
        ...
        if cmd == "add":
            ...
            if cli_opts.opt_sync_github_prs:
                machete_client.sync_annotations_to_prs(GitHubClient.config(), include_urls=False)
            elif cli_opts.opt_sync_gitlab_mrs:
                machete_client.sync_annotations_to_prs(GitLabClient.config(), include_urls=False)
            else:
                branch = get_local_branch_short_name_from_arg_or_current_branch(cli_opts.opt_branch, git)
            ...
            if 'checkout_my_github_prs' in parsed_cli:
                machete_client.checkout_pull_requests(GitHubClient.config(), pr_numbers=[], mine=True)
            ...
            config = GitHubClient.config() if cmd == "github" else GitLabClient.config()
