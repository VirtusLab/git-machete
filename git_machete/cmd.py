...
def derive_github_org_and_repo(cli_ctxt: CommandLineContext) -> Tuple[str, str]:
    from git_machete.github import parse_github_remote_url

    url_for_remote: Dict[str, str] = {
        r: get_url_of_remote(cli_ctxt, r) for r in remotes(cli_ctxt)
    }
    ...
    org_name_for_github_remote: Dict[str, Tuple[str, str]] = {
        remote: org_name for remote, org_name in optional_org_name_for_github_remote.items() if org_name
    }
    ...
    if len(org_name_for_github_remote) == 1:
        return list(org_name_for_github_remote.values())[0]

    if 'origin' in org_name_for_github_remote:
        return org_name_for_github_remote['origin']
...
def create_github_pr(cli_ctxt: CommandLineContext, head: str, draft: bool) -> None:
    ...
    org: str
    repo: str
    org, repo = derive_github_org_and_repo(cli_ctxt)
    ...
    if not description:
        print(fmt(f'Note: you can put a default description into <b>{description_path}</b> '
                  f'to have it automatically set up in PRs created with `git machete hub create-pr`.'))
    ...
    if milestone:
        ...
        print(fmt(f'Note: you can put a default milestone number into <b>{milestone_path}</b> '
                  f'to have it automatically set up in PRs created with `git machete hub create-pr`.'))
    ...
    if reviewers:
        ...
        print(fmt(f'Note: you can list default reviewers in <b>{reviewers_path}</b> (each GitHub login in a separate line) '
                  f'to have them automatically set up in PRs created with `git machete hub create-pr`.'))
    ...
    try:
        ...
        if cmd == "add":
            ...
            if param == "anno-prs":
                ...
                create_github_pr(cli_ctxt, current_branch(cli_ctxt), draft=cli_ctxt.opt_draft)
            elif param == "retarget-pr":
                retarget_github_pr(cli_ctxt, current_branch(cli_ctxt))
            else:
