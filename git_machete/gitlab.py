...
from git_machete.code_hosting import (CodeHostingClient, CodeHostingConfig,
...
from git_machete.utils import compact_dict, debug, popen_cmd
...
class GitLabClient(CodeHostingClient):
    ...
    def __init__(self, config: CodeHostingConfig, domain: str, organization: str, repository: str) -> None:
        super().__init__(config, domain, organization, repository)
        self.__token: Optional[GitLabToken] = GitLabToken.for_domain(domain)

    @classmethod
    def config(cls) -> CodeHostingConfig:
        return CodeHostingConfig(
            client_class=cls,
            default_domain=cls.DEFAULT_GITLAB_DOMAIN,
            display_name='GitLab',
            git_config_subsection='gitlab',
            ...
            token_providers_message=(
                ...
                '\t3. Current auth token from the `glab` GitHub CLI\n'
            )
        )
    ...
    def __fire_gitlab_api_request(self, method: str, path: str, request_body: Optional[Dict[str, Any]] = None) -> Any:
        ...
        try:
            ...
            if err.code == http.HTTPStatus.NOT_FOUND:
                ...
                raise MacheteException(
                    ...
                    f'Provide a GitLab API token with `api` access via one of the: {self._config.token_providers_message} '
    ...
    def __get_user_id_by_username(self, username: str) -> int:
    ...
    def add_assignees_to_pull_request(self, number: int, assignees: List[str]) -> None:
        assignee_ids: List[int] = [self.__get_user_id_by_username(assignee) for assignee in assignees]
    ...
    def add_reviewers_to_pull_request(self, number: int, reviewers: List[str]) -> None:
        reviewer_ids: List[int] = [self.__get_user_id_by_username(reviewer) for reviewer in reviewers]
    ...
    def get_open_pull_requests(self) -> List[PullRequest]:
        mrs = self.__fire_gitlab_api_project_request(method='GET', path_suffix='/merge_requests?state=opened')
        return [self.__get_merge_request_from_json(mr) for mr in mrs]
