...
from .code_hosting import (CodeHostingClient, CodeHostingConfig,
...
class MacheteClient:
    ...
    def sync_annotations_to_prs(self, config: CodeHostingConfig, include_urls: bool) -> None:
        domain = self.__derive_code_hosting_domain(config)
        org_repo_remote = self.__derive_org_repo_and_remote(config, domain=domain)
        code_hosting_client = config.create_client(
            domain=domain, organization=org_repo_remote.organization, repository=org_repo_remote.repository)
        print(f'Checking for open {config.display_name} {config.pr_short_name}s... ', end='', flush=True)
        current_user: Optional[str] = code_hosting_client.get_current_user_login()
        debug(f'Current {config.display_name} user is ' + (bold(current_user or '<none>')))
        all_open_prs: List[PullRequest] = code_hosting_client.get_open_pull_requests()
        print(fmt('<green><b>OK</b></green>'))
        self.__sync_annotations_to_branch_layout_file(config, all_open_prs, current_user, include_urls=include_urls, verbose=True)

    def __pull_request_annotation(self, config: CodeHostingConfig,
                                  pr: PullRequest, current_user: Optional[str], include_url: bool = False) -> str:
        anno = pr.display_text(fmt=False)
        if current_user != pr.user:
            anno += f" ({pr.user})"
        config_key = git_config_keys.annotate_with_urls(config.git_config_subsection)
        if include_url or self.__git.get_boolean_config_attr(key=config_key, default_value=False):
            anno += f" {pr.html_url}"
        return anno

    def __sync_annotations_to_branch_layout_file(self, config: CodeHostingConfig, prs: List[PullRequest], current_user: Optional[str],
        ...
        for pr in prs:
            if LocalBranchShortName.of(pr.head) in self.managed_branches:
                ...
                anno: str = self.__pull_request_annotation(config, pr, current_user, include_urls)
                ...
                if pr.base != upstream_tracking_branch:
                    ...
                    anno += f" WRONG {config.pr_short_name} BASE or MACHETE PARENT? {config.pr_short_name} has {pr.base}"
    ...
    def checkout_pull_requests(self,
        ...
        domain = self.__derive_code_hosting_domain(config)
        org_repo_remote = self.__derive_org_repo_and_remote(config, domain=domain)
        code_hosting_client = config.create_client(domain=domain, organization=org_repo_remote.organization,
                                                   repository=org_repo_remote.repository)

        current_user: Optional[str] = code_hosting_client.get_current_user_login()
        if not current_user and mine:
            msg = (f"Could not determine current user name, please check that the {config.display_name} API token provided by one of the: "
                   f"{config.token_providers_message}is valid.")
        ...
        org_repo_and_git_url_by_repo_id: Dict[int, Optional[OrganizationAndRepositoryAndGitUrl]] = {}
        for pr in sorted(applicable_prs, key=lambda x: x.number):
            if pr.head_repo_id not in org_repo_and_git_url_by_repo_id:
                org_repo_and_git_url_by_repo_id[pr.head_repo_id] = \
                    code_hosting_client.get_org_repo_and_git_url_by_repo_id_or_none(pr.head_repo_id)
            head_org_repo_and_git_url = org_repo_and_git_url_by_repo_id[pr.head_repo_id]
            if head_org_repo_and_git_url:
                head_org, head_repo, head_repo_git_url = head_org_repo_and_git_url
                ...
                warn(f'{pr.display_text()} comes from fork and its repository is already deleted. '
            ...
            path: List[PullRequest] = self.__get_path_from_pr_chain(config, pr, all_open_prs)
        ...
        debug(f'Current {config.display_name} user is ' + (current_user or '<none>'))
        self.__sync_annotations_to_branch_layout_file(config, all_open_prs, current_user=current_user, include_urls=False, verbose=False)
        if len(applicable_prs) == 1:
            self.__git.checkout(LocalBranchShortName.of(pr.head))

    @staticmethod
    def __get_path_from_pr_chain(config: CodeHostingConfig, original_pr: PullRequest, all_open_prs: List[PullRequest]) -> List[PullRequest]:
        ...
        while pr_base:
            if pr_base in visited_head_branches:
                raise MacheteException(f"There is a cycle between {config.display_name} {config.pr_short_name}s: " +
            ...
            for pr_number in pr_numbers:
                ...
                if pr:
                    ...
                    if pr:
                        ...
                        config = code_hosting_client._config
                        raise MacheteException(f"{config.pr_short_name} {config.pr_ordinal_char}{bold(str(pr_number))} "
    ...
    def restack_pull_request(self, config: CodeHostingConfig) -> None:
        domain = self.__derive_code_hosting_domain(config)
        head = self.__git.get_current_branch()
        org_repo_remote = self.__derive_org_repo_and_remote(config, domain=domain, branch_used_for_tracking_data=head)
        code_hosting_client = config.create_client(
            domain=domain, organization=org_repo_remote.organization, repository=org_repo_remote.repository)

        prs: List[PullRequest] = code_hosting_client.get_open_pull_requests_by_head(head)
        if not prs:
            raise MacheteException(f"No {config.pr_short_name}s have `{head}` as its head")
        if len(prs) > 1:
            raise MacheteException(f"Multiple {config.pr_short_name}s have `{head}` as its head: " +
        ...
        if s in statuses_to_push:
            if current_branch in self.annotations and not self.annotations[current_branch].qualifiers.push:
                warn(f'Branch <b>{current_branch}</b> is marked as `push=no`; skipping the push.\n'
                     f'Did you want to just use `git machete {config.git_machete_command} retarget-pr`?\n')

                self.retarget_pr(config, head, ignore_if_missing=False)
            else:
    ...
    def retarget_pr(self, config: CodeHostingConfig, head: LocalBranchShortName, ignore_if_missing: bool) -> None:
        domain = self.__derive_code_hosting_domain(config)
        org_repo_remote = self.__derive_org_repo_and_remote(config, domain=domain, branch_used_for_tracking_data=head)
        code_hosting_client = config.create_client(
            domain=domain, organization=org_repo_remote.organization, repository=org_repo_remote.repository)
        ...
        if not prs:
            if ignore_if_missing:
                warn(f"no {config.pr_short_name}s have `{head}` as its head")
                return
            else:
                raise MacheteException(f"No {config.pr_short_name}s have `{head}` as its head")
        if len(prs) > 1:
        ...
        if pr.base != new_base:
            ...
            print(f'Base branch of {pr.display_text()} has been switched to {bold(new_base)}')
            pr = pr._replace(base=new_base)
        else:
            print(f'Base branch of {pr.display_text()} is already {bold(new_base)}')
        ...
        if self.__annotations.get(head) and self.__annotations[head].qualifiers_text:
            self.__annotations[head] = Annotation(f'{self.__pull_request_annotation(config, pr, current_user)} ' +
                                                  self.__annotations[head].qualifiers_text)
        else:
            self.__annotations[head] = Annotation(self.__pull_request_annotation(config, pr, current_user))
        self.save_branch_layout_file()

    def __derive_code_hosting_domain(self, config: CodeHostingConfig) -> str:
        return self.__git.get_config_attr_or_none(key=git_config_keys.domain(config.git_config_subsection)) or config.default_domain
    ...
    def __derive_org_repo_and_remote(
        ...
        remote_config_key = git_config_keys.remote(config.git_config_subsection)
        organization_config_key = git_config_keys.organization(config.git_config_subsection)
        repository_config_key = git_config_keys.repository(config.git_config_subsection)
        ...
        if remote_from_config:
            ...
            if not org_and_repo:
                raise MacheteException(f'`{remote_config_key}` git config key points to `{remote_from_config}` remote, '
                                       f'but its URL `{url}` does not correspond to a valid {config.display_name} repository')
        ...
        if org_from_config and repo_from_config:
            ...
            raise MacheteException(
                ...
                f'(organization/repository) on {config.display_name}.\n'
        ...
        if not remote_and_organization_and_repository_from_urls:
            raise MacheteException(
                ...
                f'seems to correspond to {config.display_name} (see `git remote -v` for details).\n'
                f'It is possible that you are using a custom {config.display_name} URL.\n'
                'If that is the case, you can provide repository information explicitly via some or all of git config keys: '
                f'`machete.{config.git_config_subsection}.{{domain,remote,organization,repository}}.`\n')
        ...
        raise MacheteException(
            f'Multiple non-origin remotes correspond to {config.display_name} in this repository: '
    ...
    def create_pull_request(
        ...
        try:
            self.__sync_before_creating_pr(config, opt_onto=opt_onto, opt_yes=opt_yes)
        except InteractionStopped:
            return

        base: Optional[LocalBranchShortName] = self.__up_branch.get(LocalBranchShortName.of(head))
        if not base:
            raise UnexpectedMacheteException(f'Could not determine base branch for {config.pr_short_name}. '
        ...
        if '/'.join([org_repo_remote.remote, base]) not in self.__git.get_remote_branches():
            ...
            warn(f'Base branch for this {config.pr_short_name} ({bold(base)}) is not found on remote, pushing...')
            self.__handle_untracked_branch(
                branch=base,
        ...
        force_description_from_commit_message = self.__git.get_boolean_config_attr(
            key=git_config_keys.force_description_from_commit_message(config.git_config_subsection), default_value=False)
        ...
        print(f'Creating a {"draft " if opt_draft else ""}{config.pr_short_name} from {bold(head)} to {bold(base)}... ', end='', flush=True)
        ...
        if base_branch_found_on_remote:
            ...
            if text_to_prepend:
                ...
                print(f'Updating description of {pr.display_text()} to include '
                      f'the chain of {config.pr_short_name}s... ', end='', flush=True)
    ...
    def __sync_before_creating_pr(self, config: CodeHostingConfig, *, opt_onto: Optional[LocalBranchShortName], opt_yes: bool) -> None:
        ...
        if current_branch not in self.managed_branches:
            ...
            if current_branch not in self.managed_branches:
                raise MacheteException(
                    "Subcommand `create-pr` can NOT be executed on the branch"
                    ...
                    "branch layout file during the execution of `create-pr` subcommand.")
        ...
        if not up_branch:
            raise MacheteException(
                ...
                f'base branch for the {config.pr_short_name} cannot be established.')
