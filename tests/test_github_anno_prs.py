...
class TestGitHubAnnoPRs(BaseTest):
    ...
    @staticmethod
    def github_api_state_for_test_anno_prs_retrieve_by_author() -> MockGitHubAPIState:
        return MockGitHubAPIState.with_prs(
            mock_pr_json(number=7, user='some_other_user', head='allow-ownership-link', base='develop'),
            mock_pr_json(number=31, user='github_user', head='call-ws', base='develop'),
            mock_pr_json(number=37, user='github_user', head='develop', base='master')
    ...
    def test_github_anno_prs_retrieve_by_author(self, mocker: MockerFixture) -> None:
        self.patch_symbol(mocker, 'git_machete.github.GitHubToken.for_domain', mock_github_token_for_domain_fake)
    ...
    def test_github_anno_prs_retrieve_by_author_no_current_user(self, mocker: MockerFixture) -> None:
        ...
        assert_failure(
            ["github", "anno-prs"],
            """
            machete.github.retrieveByAuthor git config key is set, but the current GitHub user could not be determined.
