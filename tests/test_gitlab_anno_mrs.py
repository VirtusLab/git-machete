...
class TestGitLabAnnoMRs(BaseTest):
    ...
    @staticmethod
    def gitlab_api_state_for_test_anno_mrs_retrieve_by_author() -> MockGitLabAPIState:
        return MockGitLabAPIState.with_mrs(
            mock_mr_json(number=7, user='some_other_user', head='allow-ownership-link', base='develop'),
            mock_mr_json(number=31, user='gitlab_user', head='call-ws', base='develop'),
            mock_mr_json(number=37, user='gitlab_user', head='develop', base='master')
    ...
    def test_gitlab_anno_mrs_retrieve_by_author(self, mocker: MockerFixture) -> None:
        self.patch_symbol(mocker, 'git_machete.gitlab.GitLabToken.for_domain', mock_gitlab_token_for_domain_fake)
        self.patch_symbol(mocker, 'urllib.request.urlopen', mock_urlopen(self.gitlab_api_state_for_test_anno_mrs_retrieve_by_author()))
        ...
        # With `retrieveByAuthor` set, only MRs authored by the current user (gitlab_user) are downloaded and annotated;
