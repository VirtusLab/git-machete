...
from tests.mockers import (assert_failure, assert_success, execute,
                           ...
                           set_file_executable, sleep, write_to_file)
...
class TestGitHubCreatePR(BaseTest):
    ...
    def test_github_create_pr_with_description_intro_hook(self, mocker: MockerFixture) -> None:
        self.patch_symbol(mocker, 'git_machete.github.GitHubToken.for_domain', mock_github_token_for_domain_fake)
        self.patch_symbol(mocker, 'git_machete.code_hosting.OrganizationAndRepository.from_url', mock_from_url)
        # Create a GitHub API state with an existing PR to trigger description generation
        github_api_state = MockGitHubAPIState.with_prs(
            mock_pr_json(head='develop', base='master', number=1)
        )
        self.patch_symbol(mocker, 'urllib.request.urlopen', mock_urlopen(github_api_state))
        ...
        # Create a hook that outputs custom PR description intro
        hook_content = textwrap.dedent("""
            #!/bin/sh
            # Hook receives: PR_NUMBER HEAD_BRANCH BASE_BRANCH PR_TITLE
            echo "Custom intro for PR #$1"
            echo "Branch: $2 -> $3"
            echo "Title: $4"
            echo ""
            echo "This is a custom description generated by the hook!"
        """).strip()

        write_to_file(".git/hooks/machete-pr-description-intro", hook_content)
        set_file_executable(".git/hooks/machete-pr-description-intro")

        check_out("feature")
        launch_command("github", "create-pr", "--title", "Test PR with hook")

        # Verify that the PR was created with the hook's output
        pr = github_api_state.get_pull_by_number(2)
        assert pr is not None
        assert pr['title'] == "Test PR with hook"

        # Check that the description contains the hook's output wrapped in git-machete comments
        expected_description = textwrap.dedent("""
            <!-- start git-machete generated -->

            Custom intro for PR #2
            Branch: feature -> develop
            Title: Test PR with hook

            This is a custom description generated by the hook!

            <!-- end git-machete generated -->
        """).strip() + '\n'  # The system adds a trailing newline

        assert pr['body'] == expected_description
