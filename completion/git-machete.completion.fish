...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from add; and not __fish_seen_subcommand_from --as-root -R" -f -l onto -s o -r -a '(__fish_git_local_branches)' -d 'Specifies the target parent branch to add the given branch onto. Cannot be specified together with -R/--as-root'
...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from anno"                 -f -l branch -s b -r -a '(__fish_git_local_branches)'  -d 'Branch to set the annotation for'
...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from discover"                                                                  -f -l roots             -s r -r -a '(__fish_git_local_branches)' -d 'Comma-separated list of branches that should be considered roots of trees of branch dependencies. If not present, master is assumed to be a root. Note that in the process of discovery, certain other branches can also be additionally deemed to be roots as well'
...
# form 2
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from fork-point; and not __fish_seen_subcommand_from --inferred --unset-override --override-to --override-to-inferred --override-to-parent" -f -l override-to          -r
...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from github; and not __fish_seen_subcommand_from anno-prs checkout-prs create-pr retarget-pr" -f -a retarget-pr -d 'Sets the base of the current branch\'s PR to upstream (parent) branch, as seen by git machete (see git machete show up)'
...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from reapply" -f -l fork-point -s f -r -a '(__fish_git_local_branches)' -d 'Specifies the alternative fork point commit after which the rebased part of history is meant to start'
...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from slide-out; and not __fish_seen_subcommand_from --merge -M --down-fork-point -d"                                -f -l down-fork-point -s d -r -d 'If updating by rebase, specifies the alternative fork point for downstream branches for the operation. git machete fork-point overrides for downstream branches are recommended over use of this option. See also doc for --fork-point option in git machete help reapply and git machete help update. Not allowed if updating by merge'
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from slide-out; and not __fish_seen_subcommand_from --merge -M"                                                     -f -l merge           -s M                     -d 'Update the downstream branch by merge rather than by rebase'
...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from slide-out; and not __fish_seen_subcommand_from -n --no-edit-merge --no-interactive-rebase; and not __fish_seen_subcommand_from -M --merge" -f -l no-interactive-rebase    -d 'If updating by rebase, run git rebase in non-interactive mode (without -i/--interactive flag). Not allowed if updating by merge'
...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from squash" -f -l fork-point -s f -r -a '(__fish_git_local_branches)' -d 'Specifies the alternative fork point commit after which the squashed part of history is meant to start'
...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from traverse; and not __fish_seen_subcommand_from --return-to"         -f -l return-to  -r -a 'stay here nearest-remaining' -d 'Specifies the branch to return after traversal is successfully completed; WHERE can be here (the current branch at the moment when traversal starts), nearest-remaining (nearest remaining branch in case the here branch has been slid out by the traversal) or stay (the default - just stay wherever the traversal stops). Note: when user quits by q/yq or when traversal is stopped because one of git actions fails, the behavior is always stay'
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from traverse; and not __fish_seen_subcommand_from --start-from"        -f -l start-from -r -a 'here root first-root'        -d 'Specifies the branch to start the traversal from; WHERE can be here (the default - current branch, must be managed by git-machete), root (root branch of the current branch, as in git machete show root) or first-root (first listed managed branch)'
...
complete -c git -n "__fish_git_using_command machete; and __fish_seen_subcommand_from update; and not __fish_seen_subcommand_from --merge -M" -f -l fork-point -s f -r -a '(__fish_git_local_branches)' -d 'If updating by rebase, specifies the alternative fork point commit after which the rebased part of history is meant to start. Not allowed if updating by merge'
