...
complete -c git-machete -n "__fish_seen_subcommand_from gitlab; and not __fish_seen_subcommand_from anno-mrs checkout-mrs create-mr restack-mr retarget-mr sync update-mr-descriptions"    -f -a create-mr                                            -d 'Create a MR for the current branch, using the upstream (parent) branch as the MR source branch'
