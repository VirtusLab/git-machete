...
complete -c git-machete -n "__fish_seen_subcommand_from slide-out; and not __fish_seen_subcommand_from --removed-from-remote -M --merge -d --down-fork-point -n --no-edit-merge --no-interactive-rebase --no-rebase" -f -l no-rebase -d 'Skip rebase/merge of downstream branches after sliding out'
