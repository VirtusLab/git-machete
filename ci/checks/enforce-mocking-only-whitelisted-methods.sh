...
git_machete.utils.fs.find_executable
git_machete.utils.fs.get_current_date
git_machete.utils.terminal.get_terminal_height
git_machete.utils.terminal.is_stderr_a_tty
git_machete.utils.terminal.is_stdout_a_tty
git_machete.utils.terminal.is_terminal_fully_fledged
sys.argv
termios.tcgetattr
termios.tcsetattr
tty.setraw
...
if echo "$actual_methods" | exclude <(echo "$whitelisted_methods") | grep ''; then
  echo
