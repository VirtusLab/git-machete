...
docker-compose --ansi never pull deb-ppa-upload
...
image_tag=$(docker-compose --ansi never config | yq eval '.services.deb-ppa-upload.image' -)
docker image inspect "$image_tag" &>/dev/null || {
  docker-compose --ansi never build --build-arg user_id="$(id -u)" --build-arg group_id="$(id -g)" deb-ppa-upload
  ...
  docker-compose --ansi never push deb-ppa-upload || true
}
