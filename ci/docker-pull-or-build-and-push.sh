...
GROUP_ID=3000
export GROUP_ID
groupadd -g $USER_ID docker_user
useradd docker_user -u $USER_ID -g $GROUP_ID -s /bin/bash
...
docker image inspect "$image_tag" &>/dev/null || {
  docker-compose --ansi never build --build-arg user_id=$USER_ID --build-arg group_id=$GROUP_ID $image_name
